...
class TestAuthControllers:
    def test_validate_user_with_valid_credentials(self,mock_db_fetch_data, mock_hash_pwd, mock_employee_views):
    
        instance = AuthControllers()
        email = 'test@example.com'
        password = 'password123'
        
        mock_db_fetch_data.return_value = [(1, email, 'hashed_password', 'Admin', 'True')]

        result = instance.validate_user(email, password)

        assert result 

    def test_validate_user_with_invalid_credentials(self,mock_db_fetch_data, mock_hash_pwd, mock_employee_views):
    ...
    def test_valdate_user__first_time_login(self,mock_db_fetch_data,mock_change_default_pwd, mock_employee_views, mock_hash_pwd):

        instance = AuthControllers()
        email = 'test@example.com'
        password = 'valid_password'

        mock_db_fetch_data.return_value = [(1, email, 'hashed_password', 'Admin', 'False')]
    ...
    def test_change_default_pwd_positive(self,mocker, new_pwd, confirm_pwd, mock_hash_pwd, mock_db_save_data):
        ...
        mocker.patch('controllers.auth_controllers.input_valid_password', side_effect = [new_pwd, confirm_pwd])
    
        assert instance.change_default_pwd(email)


    @pytest.mark.parametrize("new_pwd, confirm_pwd", [("abcd", "abc")])
    def test_change_default_pwd_negative(self,mocker, new_pwd, confirm_pwd, mock_hash_pwd, mock_db_save_data):
