...
@pytest.fixture
def mock_obj_employee_views(mocker, mock_user_controller_class, mock_dose_info_views_class,mock_dose_controller_class):
    return EmployeeViews(1)


class TestEmployeeViews:

    def test_emp_menu(self, mocker, mock_dose_info_views_class, mock_obj_employee_views):
        ...
        mocker.patch.object(mock_dose_info_views_class(),'update_dose_info', lambda a: None)
    ...
    @pytest.mark.parametrize("inputs, reminders", [([(0,"")], Prints.REMINDER_1),([(1,"")], Prints.REMINDER_2),([(1,"")], Prints.REMINDER_3),([(2,"")], Prints.REMINDER_4)])
    def test_show_reminders(self, inputs, mocker, reminders, capsys, mock_obj_employee_views):
        mocker.patch.object(mock_obj_employee_views.dose_controller_obj,'fetch_vacc_status', return_value = inputs)
        if inputs == 1:
            mocker.patch.object(mock_obj_employee_views.dose_controller_obj, 'fetch_dose_info', lambda a: "date")
    ...
    def test_update_profile(self, mock_user_controller_class, capsys, mocker, mock_obj_employee_views):
    ...
    @pytest.mark.parametrize("vacc_status, output", [([(0,"")], Prints.NOT_VACCINATED), ([(1,"")], Prints.VACC_STATUS_1), ([(2,"")], Prints.VACC_STATUS_2)])
    def test_check_vacc_status_1(self,mocker,vacc_status,output,capsys,mock_obj_employee_views):
        mocker.patch.object(mock_obj_employee_views.dose_controller_obj,'fetch_vacc_status', return_value = vacc_status)
