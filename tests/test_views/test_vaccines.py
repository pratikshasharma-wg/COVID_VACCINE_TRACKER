...
class TestVaccineViews:
    ...
    def test_view_vaccines_negative(self, mocker, capsys, mock_vaccine_controller_class):
    ...
    def test_get_vaccine_name_negative(self, mocker, mock_vaccine_controller_class, capsys):
        mocker.patch.object(VaccineViews,'view_vaccines')
        mocker.patch('builtins.input', lambda a: 1)
        mock_vaccine_controller_obj = mock_vaccine_controller_class()
        mocker.patch.object(mock_vaccine_controller_obj,'is_vaccine_present', side_effect = [False, True])
        captured = capsys.readouterr()
